...
def deal_profile3(urlList):

    driver_var=driverExtenstio("deal_related")
    ...
    for url in urlList:
        ...
        try:
            iframe_deal = driver_var.find_element(By.CSS_SELECTOR, "iframe#dealmuleAds")
            switch_frame = driver_var.switch_to.frame(iframe_deal)
        except:
            iframe_deal=None
        ...
        if iframe_deal !=None:
            try:
                src_image=driver_var.execute_script(''' return document.querySelector(".dealmule-notification-offer-wrapper >div > table >tbody> tr >td > p > img").getAttribute("src") ''' )
            except:
                src_image = None

            print(f"src_image ==============> {src_image}")

            "data:image"

            if src_image !=None and not str(src_image).startswith("data:image"):
                deal_selectors_var = 'deal_realated_present'
                Resurllist.append(deal_selectors_var)
            else:
                deal_selectors_var = "deal_realated_absent"
                Resurllist.append(deal_selectors_var)
        ...
        if deal_selectors_var == "deal_realated_present" :break
...
# print(deal_profile3(['https://cariuma.com/']))
# print(deal_profile3('https://www.fila.com/'))
